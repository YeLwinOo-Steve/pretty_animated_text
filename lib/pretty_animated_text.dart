export 'src/effects/chime_bell_text.dart' show ChimeBellText;
export 'src/effects/spring_text.dart' show SpringText;
export 'src/effects/scale_text.dart' show ScaleText;
export 'src/effects/rotate_text.dart' show RotateText;
export 'src/effects/blur_text.dart' show BlurText;
export 'src/effects/offset_text.dart' show OffsetText;

/// Animation type enum for [letter, word]
export 'src/enums/animation_type.dart' show AnimationType;

/// Animation type enum for [OffsetText] animation
export 'src/enums/slide_animation_type.dart' show SlideAnimationType;

/// Animation type enum for [RotateText] animation
export 'src/enums/rotate_animation_type.dart' show RotateAnimationType;

/// Text alignment enum
export 'src/enums/text_alignment.dart' show TextAlignment;
